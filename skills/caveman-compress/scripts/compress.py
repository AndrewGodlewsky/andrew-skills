...
def _compress_file_locked(filepath: Path) -> bool:
    ...
    for attempt in range(MAX_RETRIES):
        ...
        fixed_frontmatter, fixed_body = split_frontmatter(fixed)
        # The initial call excludes frontmatter, but repair receives the full
        # file. Structural validation does not inspect YAML values, so accepting
        # a rewritten header here could silently change configuration. Compare
        # normalized line endings only; every other character must match.
        if fixed_frontmatter.replace("\r\n", "\n") != frontmatter:
            print("❌ Fix attempt aborted: YAML frontmatter changed.")
            print("   Skipping this attempt.")
            continue
