...
The chat wrapper is tracked separately in issue #11. This complete directory
is its delivery bundle; keep all files together when packaging that wrapper.
The same entry point works from a trusted complete repository checkout.
...
| `plan` | `--cache <cache-path> --skill <source-name> --commit <full-source-commit>` | Exact source, target, original and adapted manifests; save this JSON as the plan. |
...
Before supplying `--portability-reviewed`, review the complete selected source
folder at the plan's commit/path, including scripts and resources. The flag is
...
Maintain modules under `scripts/`, then run `node scripts/build-exporter.mjs`.
The committed bundle contains every dependency and requires no npm install.
`node scripts/build-exporter.mjs --check` detects stale generated files. Bundle
changes require one plugin patch; they do not change unrelated skill versions.
