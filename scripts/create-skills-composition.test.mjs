...
import { prepareSubmission as prepareTweak } from './skill-tweak/submission.mjs';

const repo = 'AndrewGodlewsky/andrew-skills', api = `/repos/${repo}`, web = `https://github.com/${repo}`;
function server({ loseCommentReadback = false, loseIndexReadback = false, denyLabel = false } = {}) {
  let time = 0, issue, lost = false, replaced = false;
  ...
  return { calls, comments, now: () => time, sleep: async ms => { time += ms; }, authenticate: async () => {},
    async request(method, path, body) {
      ...
      if (path === `${api}/issues/7` && method === 'PATCH') { Object.assign(issue, body); replaced = true; return { status: 200, body: structuredClone(issue) }; }
      if (path === `${api}/issues/7` && method === 'GET') {
        if (loseIndexReadback && replaced && !lost) { lost = true; return { status: 404, body: {} }; }
        return { status: 200, body: structuredClone(issue) };
      }
...
for (const name of ['creator', 'tweak']) for (const lost of ['none', 'comment', 'index']) test(`${name}: real helper delivers parts and index; lost ${lost} readback`, async () => {
  const runtime = server({ loseCommentReadback: lost === 'comment', loseIndexReadback: lost === 'index' });
  const input = { title: 'Preserve intent', intent: 'Prepare a draft for review; destination remains unresolved.', interview: { status: 'recorded', text: 'Question: Draft or send? Answer: Draft.\n'.repeat(4000) } };
  const plan = name === 'creator'
    ? prepareSubmission({ ...input, specification: 'Detailed specification.\n'.repeat(6000), implementation: 'Build failed: recorded fixture limitation.', verification: 'Check failed; no claim of adoption readiness.' })
    : prepareTweak({ ...input, report: 'The requester wanted to review a draft. Destination remains unresolved; maintainer follow-up needed.' });
  ...
  for (let i = 0; i < 100; i++) {
    ...
    if (result.recoveryRequest) {
      ...
      if (lost === 'comment') assert.equal(runtime.calls.filter(x => x.method === 'PATCH').length, 0);
  ...
  assert.equal(recovered, lost !== 'none');
  ...
  assert.equal(runtime.calls.filter(x => x.method === 'PATCH').length, 1);
  assert.ok(final.includes(input.intent));
...
test('label capability failure after a real content submission does not require replay', async () => {
  ...
  const plan = prepareSubmission({ title: 'New skill', intent: 'Preserve the supplied sample behavior.', interview: { status: 'none', text: 'The initial request supplied these requirements; no questions were needed.' }, specification: 'Defined behavior.', implementation: 'None produced.', verification: 'Not run.' });
