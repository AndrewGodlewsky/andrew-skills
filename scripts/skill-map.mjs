...
const textExtensions = new Set(['.md', '.mjs', '.js', '.ts', '.json', '.yaml', '.yml', '.txt', '.py', '.sh', '.ps1', '.html', '.css', '.toml', '.xml', '.svg', '.csv', '.rst']);
...
function normalizedBytes(path, bytes) {
  ...
  if ((textExtensions.has(extname(path)) || !extname(path)) && !bytes.includes(0)) {
    try { return Buffer.from(new TextDecoder('utf-8', { fatal: true }).decode(bytes).replaceAll('\r\n', '\n')); }
    catch { /* Non-UTF-8 resources keep their exact bytes. */ }
  }
  return bytes;
}
...
export function scanCandidates(files, names, paths = [...files.keys()].filter(owner)) {
  ...
  for (const path of sorted(paths)) {
    ...
    for (const match of content.matchAll(/(?:[/$]gt:|(?<![a-z0-9_-])skills\/)([a-z0-9]+(?:-[a-z0-9]+)*)(?![a-z0-9_-])/g)) {
...
export function analyzeSkillMap(files, records) {
  ...
  for (const record of records.provenance) {
    ...
    for (const path of new Set([...record.sources, record.builder])) edges.push({
      id: `source:${record.copy}:${path}`, from: fileId(record.copy), to: fileId(path), kind: path === record.builder ? 'build' : 'source',
  ...
  function scope(name) {
    ...
    const pending = [...paths].map(path => [path, 'runtime']);
    const visited = new Set();
    for (const edge of edges.filter(edge => edge.from === skillId(name))) {
      for (const item of edge.evidence) { paths.add(item.path); pending.push([item.path, 'source']); }
    }
    while (pending.length) {
      const [path, mode] = pending.pop(), key = `${mode}:${path}`;
      if (visited.has(key)) continue;
      visited.add(key);
      for (const edge of edges.filter(edge => edge.from === fileId(path) && edge.to.startsWith('file:'))) {
        if (mode === 'source' && !['source', 'build'].includes(edge.kind)) continue;
        const nextMode = edge.kind === 'source' ? 'source' : edge.kind === 'build' ? 'build' : mode;
        const target = edge.to.slice(5);
        paths.add(target); pending.push([target, nextMode]);
        for (const item of edge.evidence) paths.add(item.path);
      }
    }
    return sorted(paths);
  }
  for (const name of names) {
    ...
    const digest = hash(stable({ sources: paths.map(path => [path, sourceDigest(files, path)]),
      ...
      provenance: records.provenance.filter(item => paths.includes(item?.copy)),
    }));
    const review = Object.hasOwn(records.reviews, name) ? records.reviews[name] : undefined;
    ...
    Object.assign(node(skillId(name)), { review: status, sourceDigest: digest, sourcePaths: paths,
      reviewedSkillIndependent: status === 'reviewed' && !edges.some(edge => edge.kind === 'skill' && edge.from === skillId(name)) });
...
export function reverseImpact(map, target, { expanded = false, maxPaths = 10000 } = {}) {
  ...
  function visit(current, path, seen) {
    for (const edge of edges.filter(item => item.to === current)) {
      ...
      // Reusing a source file does not inherit its original runtime dependencies:
      // a builder may rewrite imports/links. Use the generated copy's edges for
      // runtime use, and continue only provenance/build inputs after a source hop.
      let mode = 'runtime', valid = true;
      for (const step of next) {
        if (step.kind === 'source') mode = 'source';
        else if (step.kind === 'build') mode = 'build';
        else if (mode === 'source') { valid = false; break; }
      }
      if (!valid) continue;
