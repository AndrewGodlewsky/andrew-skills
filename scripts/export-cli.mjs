...
import { acquireCatalog, checkRuntime, executePlan, makePlan, reviewPlan } from './export-protocol.mjs';
...
export function argumentsFor(argv) {
  const { values, positionals } = parseArgs({ args: argv, allowPositionals: true, strict: true, options: {
    ...
    plan: { type: 'string' }, name: { type: 'string' }, 'portability-reviewed': { type: 'boolean' }, 'review-source': { type: 'boolean' },
  } });
  ...
  const allowed = { list: ['checkout', 'offline-cache'], plan: ['cache', 'skill', 'commit', 'review-source'],
...
export function runCommand(argv) {
  ...
  if (command === 'plan') return (values['review-source'] ? reviewPlan : makePlan)({ target, cache: values.cache, skill: values.skill, commit: values.commit });
