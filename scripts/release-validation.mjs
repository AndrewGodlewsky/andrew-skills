...
function snapshotInfo(files, legacyAllowed) {
  ...
  for (const [path, file] of files) {
    if (!path.startsWith('skills/') && !['plugin.json', '.claude-plugin/marketplace.json'].includes(path)) continue;
  ...
  return { skills, version, config, legacy: skills.size > 0 && releaseCount === 0 };
}
...
export function validateReleaseChange(beforeFiles, afterFiles, { baseCommit, currentMainCommit } = {}) {
  ...
  const bundleChanged = changedSkills.length + addedSkills.length + removedSkills.length > 0 || before.config !== after.config;
